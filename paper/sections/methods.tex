In this section, we first outline how we obtain least-cost and near-optimal
solutions for a given cost parameter set. We then describe the model of the
European power system and define the cost uncertainties. Finally, we explain how
we make use of multi-fidelity surrogate modelling techniques based on polynomial
chaos expansions and find an experimental design that efficiently covers the
parameter space.
...
\usubsection{Near-Optimal Alternatives}{sec:nearoptimal}
...
\begin{SCfigure}
        \begin{tabular}{cc}
            ...
            \includegraphics[width=0.31\textwidth]{map37.pdf} &
            \includegraphics[width=0.31\textwidth]{map128.pdf} \\
            \includegraphics[width=0.31\textwidth]{timeseries37.pdf} &
            \includegraphics[width=0.31\textwidth]{timeseries128.pdf} \\
        \end{tabular}
...
\usubsection{Experimental Design for Surrogate Model}{sec:sampling}
...
\usubsection{Validation of Surrogate Model}{sec:validation}
...
\usubsection{Critical Appraisal}{sec:sub:limitations}
