...
The instances of the coordinated capacity expansion problem
 (\cref{sec:leastcost}) are based on \mbox{PyPSA-Eur}, which is an open model of
...
infeasible. Throughout the paper, we will therefore make use of two levels of
...
As modelling technological learning endogeneously is computationally challenging due to the nonconvexity it entails \cite{heuberger_power_2017,mattsson_learning_2019},
...
In the following, we will formalise this approach mathematically,
...
distribution. In \cref{sec:inputs} we already alluded to using two models with
...
based on fewer samples analogous to \cref{sec:surrogate}.
