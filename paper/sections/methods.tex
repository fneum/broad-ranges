...
The objective of long-term power system planning is to minimise the total annual
...
The dispatch of a renewable generator is constrained by its rated capacity and
...
Kichhoff's Voltage Law (KVL) imposes further constraints on the flow of AC
...
b$ the set of linear constraints \crefrange{eq:firstA}{eq:lastA} in a space of
...
We then encode the original objective as a constraint such that the cost
...
The instances of the coordinated capacity expansion problem (see
 \nameref{sec:leastcost}) are based on \mbox{PyPSA-Eur}, which is an open model
...
We determine these coefficients by a regression based on a set of cost parameter
...
regularisation term, % LASSO regression
...
distribution. In the section on \nameref{sec:inputs} we already alluded to using
...
the experimental design (see \nameref{sec:sampling}). With $p_\Delta < p_\ell$
...
this choice by experimentation in the \nameref{sec:validation}, by testing
...
slow convergence properties, % 1 / \sqrt{n}
...
Regarding the number of samples required, \cref{fig:error:samples} foremost
